...
class LyricWidget extends CustomPainter with ChangeNotifier {
  ...
  LyricWidget(this.lyric, this.curLine) {
    ...
    lyricPaints.addAll(lyric
        .map((l) => TextPainter(
            text: TextSpan(text: l.lyric, style: R.style.commonGrayTextStyle),
      ...
      if (y > size.height || y < (0 - lyricPaints[i].height / 2)) {
        ...
        if (curLine == i) {
          ...
          lyricPaints[i].text = TextSpan(
              text: lyric[i].lyric, style: R.style.commonWhiteTextStyle);
          ...
          // 如果是拖动状态中的当前行
          lyricPaints[i].text = TextSpan(
              text: lyric[i].lyric, style: R.style.commonWhite70TextStyle);
          lyricPaints[i].layout();
        } else {
          lyricPaints[i].text = TextSpan(
              text: lyric[i].lyric, style: R.style.commonGrayTextStyle);
          lyricPaints[i].layout();
        }
    ...
    if (isDragging) {
      ...
      draggingLineTimeTextPainter = TextPainter(
        text: TextSpan(
            ...
            style: R.style.smallGrayTextStyle),
        textDirection: TextDirection.ltr,
      );
